document.addEventListener("DOMContentLoaded", function (e) {
  ...
  const imgSliderPrev = document.querySelectorAll(
    "ul.sliderControl button.prev"
  );
  const imgSliderNext = document.querySelectorAll(
    "ul.sliderControl button.next"
  );
  ...
  const ideafestival = scrollMonitor.create(
    document.querySelectorAll("div.title.ideafestival")
  );
  const audiotechnica = scrollMonitor.create(
    document.querySelectorAll("div.title.at")
  );
  const shopick = scrollMonitor.create(
    document.querySelectorAll("div.title.shopick")
  );
  const brothumb = scrollMonitor.create(
    document.querySelectorAll("div.title.brothumb")
  );

  ideafestival.enterViewport(function () {
    for (let i = 0; i < 4; i++) {
      document
        .querySelector("div.slider")
        .children[i].querySelector("img").src =
        "./common/images/img_ideafestival0" + (i + 1) + ".jpg";
  ...
  shopick.enterViewport(function () {
    for (let i = 0; i < 4; i++) {
      document
        .querySelector("div.slider")
        .children[i].querySelector("img").src =
        "./common/images/img_shopick_0" + (i + 1) + ".jpg";
  ...
  brothumb.enterViewport(function () {
    for (let i = 0; i < 4; i++) {
      document
        .querySelector("div.slider")
        .children[i].querySelector("img").src =
        "./common/images/img_brothumb0" + (i + 1) + ".jpg";
